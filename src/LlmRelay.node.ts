...
import type { ChatMessage } from './transport/types.js';
...
export interface CredentialData {
  openAiApi?: { apiKey: string };
  openAiCompat?: { apiKey: string; baseURL: string };
  deepseekCompat?: { apiKey: string; baseURL: string };
}

function renderTemplate(template: string, vars: Record<string, unknown>, userText: string): string {
  let result = template;
...
export async function relay(
  ...
  let upstreamMessages: ChatMessage[] = [];
  if (upstream.systemPrompt) upstreamMessages.push({ role: 'system', content: upstream.systemPrompt });
  ...
  if (shouldSendDownstream) {
    ...
    if (downstream.provider === 'openai') {
      ...
      const cred =
        downstream.provider === 'openaiCompat'
          ? creds.openAiCompat
...
export class LlmRelay implements INodeType {
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    const item = items[0].json as unknown as RelayInputs;
    const upstream = this.getNodeParameter('upstream', 0) as UpstreamConfig;
    const downstream = this.getNodeParameter('downstream', 0) as DownstreamConfig;
    const routing = this.getNodeParameter('routing', 0) as RoutingConfig;
    const output = this.getNodeParameter('output', 0) as OutputConfig;

    const creds: CredentialData = {
      openAiApi: (await this.getCredentials('openAiApi').catch(() => undefined)) as any,
      openAiCompat: (await this.getCredentials('openAiCompat').catch(() => undefined)) as any,
      deepseekCompat: (await this.getCredentials('deepseekCompat').catch(() => undefined)) as any,
    };
